...
from(arr).subscribe({
  ...
  error: (valor: string) => console.log('Erro: ' + valor),
...
console.log('\nobs1');
const obs1 = from(valores).pipe(filter((valor: number) => valor % 2 == 0));
obs1.subscribe({
  next(valor: number) {
    console.log(valor);
  },
...
console.log('\nobs5');
const obs5 = from(valores).pipe(count((valor: number) => valor % 2 == 0));
obs5.subscribe({
  next(valor: number) {
    console.log(valor);
  },
